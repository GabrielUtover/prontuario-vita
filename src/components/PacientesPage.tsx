...
export function PacientesPage() {
  ...
  const pacienteDropdown = openActionsDropdownId
    ? pacientes.find(p => p.id === openActionsDropdownId) || null
    : null
  ...
  return (
    <div className="pacientes-page">
      ...
      {/* Popup de ações do paciente - cobre toda a página */}
      {view === 'list' && pacienteDropdown && (
        <div
          className="actions-popup-overlay"
        ...
        >
          ...
          >
            <div className="actions-popup-header">
              <h3>Ações para {pacienteDropdown.nome}</h3>
              <button
                type="button"
                className="actions-popup-close"
                onClick={() => setOpenActionsDropdownId(null)}
                aria-label="Fechar"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="actions-popup-body">
              <button
                type="button"
                className="action-popup-btn primary"
                onClick={() => handleAddAgendaDia(pacienteDropdown)}
                disabled={estaNaAgendaHoje(pacienteDropdown.id)}
              >
                ...
                {estaNaAgendaHoje(pacienteDropdown.id) ? 'Já está na agenda do dia' : 'Colocar na agenda do dia'}
              </button>
              <button
                type="button"
                className="action-popup-btn"
                onClick={() => { handleViewPaciente(pacienteDropdown); setOpenActionsDropdownId(null); }}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <button
                type="button"
                className="action-popup-btn"
                onClick={() => { handleEditPaciente(pacienteDropdown); setOpenActionsDropdownId(null); }}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <button
                type="button"
                className="action-popup-btn"
                onClick={() => { handleToggleStatus(pacienteDropdown); setOpenActionsDropdownId(null); }}
              >
                {pacienteDropdown.status === 'ativo' ? (
                  <>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <circle cx="12" cy="12" r="10"/>
                      <line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/>
              ...
              <button
                type="button"
                className="action-popup-btn danger"
                onClick={() => { setDeleteConfirm(pacienteDropdown.id); setOpenActionsDropdownId(null); }}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <polyline points="3,6 5,6 21,6"/>
                  <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
